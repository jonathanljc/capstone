...
# 2) one LOS row
los_df = df[df["NLOS"] == 0.0]
if los_df.empty: raise ValueError("No LOS rows where NLOS == 0.0")
row = los_df.iloc[0]
...
print(f"LOS sample FP_IDX {fp_idx}")
...
# 5) Powers from registers (DW1000 equations)
EPS = 1e-12
N = rxpacc
...
RFP_reg      = RX_POWER_reg - FP_POWER_reg      # ≡ 10*log10((C_reg*2^17)/FP_reg)

# sanity check: total vs first-path energy in register domain
ratio_reg = (C_reg * (2**17)) / (FP_reg + EPS)
print(f"Register check: C_reg*2^17 / FP_sum = {ratio_reg:.6g}  (RFP_reg {RFP_reg:.2f} dB)")

# If register RFP is negative, fall back to same-domain CIR ratio for classification
use_reg = RFP_reg >= 0.0
if use_reg:
    RX_POWER, FP_POWER, RFP_metric = RX_POWER_reg, FP_POWER_reg, RFP_reg
    rfp_mode = "reg"
else:
    # same-domain ratio from raw CIR samples
    C_cir  = float(np.sum(cir_raw**2))
    w0, w1 = max(0, fp_idx - 1), min(n, fp_idx + 2)   # 3-sample FP window
...
ax.plot(idx, cir_plot, color=COL["los"], lw=2, label="LOS CIR")
...
ax.set_title(f"LOS (FP_IDX {fp_idx}, TFP {tfp}, RFP {RFP_metric:.2f} dB, {rfp_class}, {title_tail})")
ax.set_xlabel("Index")
...
legend_handles = [
    Line2D([0], [0], color=COL["los"], lw=2, label="LOS CIR"),
...
axins.plot(idx, cir_plot, color=COL["los"], lw=2)
if SHOW_FPIDX:
...
plt.savefig("los_cir_register_checked.png", dpi=300, bbox_inches="tight")
plt.show()
